  ...
  if (parameters["c"].type() == Value::Type::VECTOR) {
    ...
    for (size_t i = 0; i < 4; ++i) {
      ...
      if (node->color[i] > 1 || node->color[i] < 0) {
        LOG(message_group::Warning, inst->location(), parameters.documentRoot(), "part() expects numbers between 0.0 and 1.0. Value of %1$.1f is out of range", node->color[i]);
      }
    }
...
{
  return STR("part([", this->color[0], ", ", this->color[1], ", ", this->color[2], ", ", this->color[3], "])");
}
