...
{
  importer->polySet = new PolySet(3, Geometry::Attributes{}); //FIXME-MM: set up attributes properly
}
...
PolySet *AmfImporter::read(const std::string& filename)
{
  ...
  if (polySets.size() > 1) {
    ...
    if (auto ps = CGALUtils::getGeometryAsPolySet(CGALUtils::applyUnion3D(children.begin(), children.end(), Geometry::Attributes{}))) { //FIXME-MM: remove union and set up attributes properly
      p = new PolySet(*ps);
    } else {
