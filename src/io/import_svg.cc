                      ...
                      const double dpi, const bool center, const Location& loc)
{
  try {
    ...
    for (const auto& shape_ptr : *shapes) {
      if (!shape_ptr->is_excluded()) {
        Polygon2d *poly = new Polygon2d(Geometry::Attributes{}); //FIXME-MM: maybe we could extract metadata here, especially color
        const auto& s = *shape_ptr;
        for (const auto& p : s.get_path_list()) {
          Outline2d outline;
          for (const auto& v : p) {
    ...
    libsvg_free(shapes);
    return ClipperUtils::apply(polygons, ClipperLib::ctUnion, Geometry::Attributes{}); //FIXME-MM: maybe we could extract metadata here, especially color
  } catch (const std::exception& e) {
    LOG(message_group::Error, Location::NONE, "", "%1$s, import() at line %2$d", e.what(), loc.firstLine());
    return new Polygon2d(Geometry::Attributes{});
  }
}
